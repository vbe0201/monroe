...
/// on demand, e.g. when restarted by a supervisor.
pub trait NewActor: Send + 'static {
    /// The [`Actor`] type produced by this factory.
    type Actor: Actor;

    /// The argument that is passed to [`NewActor::make`]
    /// for creating new actors.
    ///
    /// For the initial instance, the value is provided by
    /// the user directly. For subsequent restarts of failed
    /// actors, the [`Supervisor`] must provide them.
    type Arg;
    ...
    /// Attempts to create a new [`Actor`] from the supplied
    /// argument.
    ///
    /// Failures to create new actors will be forwarded to
    /// [`Supervisor::on_restart_error`] or
    /// [`Supervisor::on_second_restart_error`].
    fn make(
        &mut self,
